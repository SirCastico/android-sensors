...
/*
 * modified, based on a file on https://github.com/google-ar/arcore-android-sdk
 */
...
public final class PointCloudHelper {
    ...
    public static FloatBuffer convertDepthTo3dCameraSpacePointBufferFilteredConfidence(
            Image depth, Image confidence, CameraIntrinsics cameraTextureIntrinsics, int pointLimit, float confidenceThresh,
            float[] transform, Collection<com.google.ar.core.Plane> planes) {
        Plane depthImagePlane = depth.getPlanes()[0];
        // Set the endianess to ensure we extract depth data in the correct byte order.
        ShortBuffer depthBuffer =
        ...
        for (int y = 0; y < depthHeight; y += step) {
            for (int x = 0; x < depthWidth; x += step) {
                ...
                float confidenceNormalized = ((float) (confidencePixelValue & 0xff)) / 255.0f;
                if(confidenceNormalized<confidenceThresh){
                    continue;
                }
    ...
    public static FloatBuffer convertImageToColorBufferPoints(
        ...
        FloatBuffer colors =
                FloatBuffer.allocate(
                        depthWidth / step * depthHeight / step * COLOR_FLOATS_PER_POINT);
